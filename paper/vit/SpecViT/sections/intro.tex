\begin{abstract}
Large spectroscopic surveys require scalable inference of stellar atmospheric parameters from noisy spectra. We present SpecViT, a Vision Transformer (ViT) regressor for stellar surface gravity (log g) from medium-resolution, one-dimensional spectra. SpecViT partitions each median-normalized spectrum into contiguous wavelength patches, embeds them as tokens with learned positional encodings, and uses global self-attention to combine information across the bandpass. We train on one million synthetic spectra generated from the BOSZ atmosphere grid and simulated for the Subaru Prime Focus Spectrograph (PFS) medium-resolution (MR) arm (710--885 nm; 4096 pixels) with heteroscedastic per-pixel uncertainties and on-the-fly noise injection. On a held-out test set of 10,000 spectra, a fixed-capacity configuration with 16-pixel patches and a 6-layer, 256-wide encoder reaches a coefficient of determination of R2=0.711, with mean absolute error (MAE) 0.372 dex and root-mean-square error (RMSE) 0.64 dex, outperforming LightGBM (R2=0.614), ridge regression (R2=0.50), and template fitting (R2=0.404) on the same split. Performance improves rapidly with training set size, surpassing LightGBM near 100,000 spectra and saturating beyond 500,000 for this model. Using a Fisher-information Cram\'er--Rao lower bound (CRLB) analysis, we derive a signal-to-noise ratio (SNR)-conditioned ceiling and find that SpecViT operates within about 0.02 in R2 of the five-parameter (5D) ceiling at SNR $\approx$ 4.6, providing a physics-grounded benchmark for learned estimators.
\end{abstract}
...
\begin{minipage}[t]{0.48\textwidth}
    ...
    \IfFileExists{figs/vit_scaling_curve.png}{%
      \includegraphics[width=\linewidth]{figs/vit_scaling_curve.png}%
    }{%
      \fbox{\parbox[c][0.18\textheight][c]{0.9\linewidth}{\centering Missing figure: \texttt{vit\_scaling\_curve.png}}}%
    }
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
    ...
    \IfFileExists{figs/r2_vs_snr_ceiling_test_10k_unified_snr.png}{%
      \includegraphics[width=\linewidth]{figs/r2_vs_snr_ceiling_test_10k_unified_snr.png}%
    }{%
      \fbox{\parbox[c][0.18\textheight][c]{0.9\linewidth}{\centering Missing figure: \texttt{r2\_vs\_snr\_ceiling\_test\_10k\_unified\_snr.png}}}%
    }
\end{minipage}
\caption{
\textbf{Left:} Scaling of $\log g$ inference performance with training set size $N$. \textsc{SpecViT} improves rapidly with data and overtakes LightGBM at $N\sim10^5$, then saturates beyond $N\gtrsim5\times10^5$, suggesting a transition to a model-limited regime for the fixed-capacity configuration (motivating scalable-capacity variants such as mixture-of-experts).
\textbf{Right:} Test-set $R^2$ for $\log g$ inference as a function of signal-to-noise ratio (SNR). \textsc{SpecViT} remains the best-performing learned model across SNR and approaches the Fisher-information--based theoretical ceiling (dashed) at low-SNR conditions.
}
