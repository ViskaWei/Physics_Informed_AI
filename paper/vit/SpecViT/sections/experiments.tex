...
Figure~\ref{fig:fisher_residual_overlay} provides direct visual confirmation of this near-optimal behavior by overlaying the empirical residual scatter against CRLB envelopes at two representative noise regimes. The measured RMSE $\sigma_{\mathrm{ViT}}=0.64~\mathrm{dex}$ lies between the Fisher lower bounds at different magnitude bins ($\sigma_{\mathrm{Fisher}}\approx 0.43~\mathrm{dex}$ at mag~21.5 and $\approx 1.11~\mathrm{dex}$ at mag~22.5), demonstrating that \textsc{SpecViT} achieves performance consistent with the information-theoretic constraints across the SNR range.
...
\caption{Residual diagnostics for \textsc{SpecViT} trained on $10^6$ spectra. Empirical residuals are overlaid with CRLB envelopes at two noise regimes (mag~$\approx 21.5$ and mag~$\approx 22.5$). The measured RMSE $\sigma_{\mathrm{ViT}}=0.64~\mathrm{dex}$ falls between the corresponding Fisher bounds, confirming that the model operates near the information-theoretic limit across the SNR range.}
\label{fig:fisher_residual_overlay}
\end{figure*}

These results indicate that, in the low-SNR regime most relevant to faint targets, the current architecture is already near the information limit imposed by the spectrum and noise model, whereas at moderate-to-higher SNR there remains measurable headroom that can be targeted by improved model capacity and/or tokenization choices. Appendix~\ref{app:fisher_extra} provides supporting details on the Fisher-information conversion and additional diagnostics.
