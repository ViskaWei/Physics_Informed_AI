#!/usr/bin/env python3
"""
Markdown preview server with syntax highlighting.
...
ROOT_DIR = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("/home/swei20/Physics_Informed_AI")
    ...
    def do_GET(self):
        try:
            ...
            if file_path.suffix.lower() == '.md' and file_path.exists():
                ...
                body = f'<h1>📁 {rel_path if rel_path != Path(".") else "Physics_Informed_AI"}</h1><ul class="dir-listing">{"".join(links)}</ul>'
