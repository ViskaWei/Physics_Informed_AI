...
| Experts | 9× Ridge（按 Teff×[M/H] 分训练，**沿用已有，不变**）                     |
| Gate 输入 | `[phys_features, quality_features]`（~20 维）                  |
| Gate 架构 | MLP [32, 16] → 9 logits，Soft routing                        |
| 训练 | **回归最优（MSE on final logg）**，不是分类 CE                          |
| 验收 | ΔR² ≥ +0.005 (vs 单塔) → 继续；ΔR² < 0 → fallback 单塔            |
| 切片 | 必须按 SNR bins (>7/4-7/2-4/≤2) 分析，看低 SNR 是否改善                  |
...
| -- | ------------------------------------------------------------- |
| 目标 | 保留双 gate 各自可解释性                                              |
| 方法 A | **Logit 相加**: $w = \mathrm{softmax}(a + b)$                  |
| 方法 B | **概率乘积**: $w \propto p \odot q$（Product-of-Experts）          |
| 验收 | 对比 MVP-4.0，若可解释性更好且性能不降 → 采用                               |

### MVP-4.2: 温度控制

| 项  | 配置                                                            |
| -- | ------------------------------------------------------------- |
| 目标 | 让 quality tower 控制 gate 的"置信度/sharpness"                     |
| 方法 | quality tower 输出温度 τ(q) ≥ 1                                   |
| 公式 | w = softmax(logits_phys / τ(q))                               |
| 物理直觉 | SNR 低 → τ 大 → 更 ensemble；SNR 高 → τ 小 → 更 routing          |
| 验收 | 低 SNR 子集 R² 改善 ≥ +0.01                                       |
