...
```text
💡 核心共识（Fisher/CRLB 分析告诉我们什么）
├─ K1 理论上限 R²_max=0.89 @ noise=1 → So what: +32% headroom，值得继续投 CNN/MoE
│  └─ Evidence: V2 规则网格，CRLB range=2.9 orders，数值自洽
├─ K2 临界 SNR≈4，SNR<2 信息悬崖 → So what: mag≥22.5 需改变策略（先验/分类）
│  └─ Evidence: Multi-Mag sweep，6 个 magnitude 单调下降
└─ K3 Schur decay≈0.69 恒定 → So what: degeneracy 中等，multi-task 可选非必须
   └─ Evidence: 所有 mag Schur≈0.68-0.69
```
