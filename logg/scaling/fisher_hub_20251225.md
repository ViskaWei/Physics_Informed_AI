...
**下一步最有价值（≤2条）**
- 🔴 **P0**：**MVP-FU-3: Residual Overlay** — 在 parity/residual 图上叠加 per-sample σ_fisher(x) 包络 → 理论下界的直观落地
...
│
└── Q7: 如何在模型诊断图上直观展示 Fisher 理论下界？ 🔆
    ├── Q7.1: σ_fisher(x) 按 logg 分箱是否揭示结构？ → ⏳ 待实验
    ├── Q7.2: 模型 residual 能否系统性低于理论下界？ → ⏳ 待验证
    └── Q7.3: median vs 90% 分位哪种可视化更有效？ → ⏳ 待实验
...
| 📗 MVP-FU-3 | [`exp/exp_scaling_fisher_residual_overlay_20251228.md`](./exp/exp_scaling_fisher_residual_overlay_20251228.md) | 🔴 Residual Overlay (图上叠加理论下界) |
...
| 2025-12-28 | 🆕 **MVP-FU-3 立项**：Residual Overlay，添加 Q7 分支 | 理论下界的直观落地 |
