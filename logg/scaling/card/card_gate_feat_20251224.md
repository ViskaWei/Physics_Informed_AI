# 📇 Knowledge Card: Gate Feature Robustness
> **Name:** Gate Feature Sanity Check | **ID:** `SCALING-20251223-gate-feat-01`  
> **Topic:** `scaling` | **Source:** `exp_scaling_gate_feat_sanity_20251224.md` | **Project:** `VIT`  
> **Author:** Viska Wei | **Date:** 2025-12-24
```
💡 物理窗口特征 (Ca II, Na I, Mg I, PCA) 在 noise=1 下 9-class 准确率 88%，远超预期  
适用：MoE Gate 输入设计
```

---

## 🎯 问题与设置

**问题**: Gate 特征在高噪声 (noise=1) 下是否仍有 bin 区分能力？

**设置**: 
- 数据: BOSZ 1M train, 1k test, noise σ=1.0
- 模型: LightGBM 9-class classifier
- 关键变量: 37 维物理窗口特征 (Ca II, Na I, Mg I, PCA)

---

## 📊 关键结果

| # | 结果 | 数值 | 配置 |
|---|------|------|------|
| 1 | 9-class Accuracy @ noise=1 | **87.8%** | 阈值 60% |
| 2 | Ca II F-statistic | 25,618 | 阈值 10 |
| 3 | 有效 SNR | 6.21 | 阈值 1.0 |
| 4 | Top Feature | PCA_1 (F=287,966) | - |
| 5 | Accuracy @ noise=2 | 75.1% | 仍远超 40% |

**Noise Sweep**:
| noise | 0.0 | 0.2 | 0.5 | 1.0 | 2.0 |
|-------|-----|-----|-----|-----|-----|
| Acc | 98.3% | 96.8% | 92.5% | 88.3% | 75.1% |

---

## 💡 核心洞见

### 🏗️ 宏观层（架构设计）

- **物理窗口特征极其鲁棒**: 即使 noise=2 也能 75% 准确率
- **PCA 特征最强**: 全谱形状在 bin 区分中最关键

### 🔧 模型层（调参优化）

- **Mg I > Ca II**: Mg I 窗口更窄，信号更集中
- **SNR 在 noise 变化时几乎不变**: 物理窗口有效降噪

### ⚙️ 工程层（实现细节）

- 37 维特征: 9 Ca II + 6 Na I + 9 Mg I + 3 聚合 + 10 PCA
- 主要混淆在相邻 Teff/[M/H] bins 之间

---

## ➡️ 下一步

| 优先级 | 任务 | 相关 experiment_id |
|--------|------|-------------------|
| 🔴 P0 | 开发 trainable soft-gate MoE | MVP-16A-2 |
| 🟡 P1 | 验证 soft routing 接近 oracle 性能 | - |

---

## 🔗 相关链接

| 类型 | 路径 |
|------|------|
| 训练仓库 | `~/VIT/` |
| 脚本 | `~/VIT/scripts/scaling_gate_feat_sanity.py` |
| 完整报告 | `logg/scaling/exp/exp_scaling_gate_feat_sanity_20251224.md` |

