...
> **Ridge 和 LightGBM 在 1M 数据 + noise=1 下分别达到 R²=0.50 和 R²=0.57，确认传统 ML 存在性能天花板；从 100k→1M 数据增益 <3%，表明大数据无法突破线性/浅层模型的表达能力瓶颈。**

### 对假设的验证

| 验证问题 | 结果 | 结论 |
|---------|------|------|
| H1.1.1: Ridge 在 1M 数据 + noise=1 下 R² < 0.6? | ✅ | R²=0.4997 < 0.6 |
...
**关键观察**：
- LightGBM R²=0.5709 略优于 Ridge R²=0.4997
...
1. **传统 ML 性能天花板确认**：即使扩展到 1M 数据，Ridge R²=0.50, LightGBM R²=0.57，远低于噪声较低时的 0.90+ 水平。
...
| 指标 | 值 | 配置/条件 |
|------|-----|----------|
| Ridge 最佳 R² | 0.4997 | 1M, σ=1, α=5000 |
...
| 5000 | **0.4997** | 0.6327 | 0.8093 | 21.1 |
...
[3/6] Running Ridge experiments...
  Ridge @ 1M, α=5000: R²=0.4997, MAE=0.6327, Time=21.1s
